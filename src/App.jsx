...
const RECIPES = [
  ...
  { name: "Salata", category: "Yan", mood: ["Hafif", "Hızlı"], needs: ["Domates", "Salatalık", "Limon"], text: "Hemen her tabağa uyum sağlar." },
...
function getMinimumMatchCount(selectedCount) {
...
function scoreRecipe(recipe, selected) {
  ...
  const exactBonus = matched.length === recipe.needs.length ? 12 : 0;
  const efficiencyBonus = recipe.needs.length ? (matched.length / recipe.needs.length) * 8 : 0;
  const score = matched.length * 20 - missing.length * 6 + exactBonus + efficiencyBonus;
...
export default function App() {
  ...
  const computeResults = (items) => {
    const minMatch = getMinimumMatchCount(items.length);
    return RECIPES
      .map((recipe) => scoreRecipe(recipe, items))
      .filter((r) => r.matchedCount >= minMatch)
  ...
  const handleAddFromInput = () => {
    ...
    if (results.length === 0) {
      setNotice("Bunlarla uygun tarif bulamadım. Bir iki malzeme daha ekleyelim.");
    } else {
      setNotice(`${results.length} öneri buldum. En mantıklıları yukarı aldım.`);
    }
  ...
  const minMatch = getMinimumMatchCount(selected.length);
  ...
  const filteredPool = RECIPES
    ...
    .filter((r) => (selected.length === 0 ? true : r.matchedCount >= minMatch))
    .sort((a, b) => b.score - a.score || b.matchedCount - a.matchedCount);

  const scoredRecipes = filteredPool.slice(0, 24);
  const favoriteRecipes = RECIPES.filter((r) => favorites.includes(r.name));

  const suggestToday = () => {
    const pool = filteredPool.length
      ? filteredPool
      : RECIPES.map((r) => scoreRecipe(r, selected));
    const ideas = [...pool].sort(() => Math.random() - 0.5).slice(0, 3);
    setDailyIdeas(ideas);
    setNotice("Bugünlük hızlı fikirler hazır.");
  };

  const suggestMenu = () => {
    const soups = RECIPES.filter((r) => r.category === "Çorba");
    const mains = RECIPES.filter((r) =>
      ["Et", "Tavuk", "Balık", "Sebze", "Bakliyat", "Fırın"].includes(r.category)
    );
    ...
    setMenuIdeas(menus);
    setNotice("Sofra önerileri hazır.");
  };
    ...
    <div style={{ minHeight: "100vh", padding: 16, color: "#292524" }}>
      <div style={{ maxWidth: 460, margin: "0 auto", display: "grid", gap: 16 }}>
        <div style={sectionStyle}>
          ...
          >
            <div>
              ...
              <p style={{ margin: "8px 0 0", color: "#57534e", lineHeight: 1.4 }}>
                Bugün ne döktürüyoruz? Dolabı söyle, fikri ben vereyim.
              </p>
            </div>
            <div
              style={{
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 16 }}>
            <button onClick={suggestToday} style={primaryBtn("#f97316")}>
              Bugün Ne Yapsam?
            </button>
            <button onClick={suggestMenu} style={primaryBtn("#292524")}>
              Bugün Sofrada Ne Olsun?
            </button>
            <button
              onClick={() => setShowFavorites(!showFavorites)}
              style={primaryBtn("#ec4899")}
            >
              {showFavorites ? "Tariflere Dön" : "Favorilerim"}
            </button>
            <button
              onClick={() => setHomeOnly(!homeOnly)}
              style={primaryBtn("#16a34a")}
            >
              {homeOnly ? "Tüm Tarifler" : "Markete Gitmeden Ne Çıkar?"}
            </button>
          </div>
        </div>
        ...
        {liveResults.length > 0 && (
          <div
            ref={resultsRef}
            style={{ ...sectionStyle, borderColor: "#fdba74", background: "#fff7ed" }}
          >
            <h2 style={{ marginTop: 0 }}>Bunlarla hemen çıkar</h2>
            <div style={{ display: "grid", gap: 10 }}>
              {liveResults.map((recipe) => (
                <div
                  key={recipe.name}
                  style={{
                    border: "1px solid #fed7aa",
                ...
                >
                  ...
                  {recipe.missing.length > 0 && (
                    <p style={{ margin: "8px 0 0", color: "#78716c", fontSize: 13 }}>
                      Eksik olanlar: {recipe.missing.join(", ")}
                    </p>
                  )}
                </div>
              ))}
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            />
            <button onClick={handleAddFromInput} style={primaryBtn("#f97316")}>
              Ekle
            </button>
          </div>
        </div>
              ...
              {items.map((item) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...chipBase,
                      borderColor: active ? "#f97316" : "#d6d3d1",
        ...
        <div style={sectionStyle}>
          ...
          {selected.length === 0 ? (
            <p style={{ color: "#78716c", margin: 0 }}>
              Henüz malzeme gelmedi. Dolap sessiz, tencere umutlu.
            </p>
          ) : (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {selected.map((item) => (
        ...
        <div style={sectionStyle}>
            ...
            {FILTERS.map((f) => (
              <button
                ...
                style={{
                  ...
                  color: activeFilter === f ? "#fff" : "#292524",
                  borderColor: activeFilter === f ? "#f97316" : "#d6d3d1",
                }}
              >
                {f}
              </button>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
            {MOODS.map((m) => (
              <button
                ...
                style={{
                  ...chipBase,
                  background: mood === m ? "#ec4899" : "#fff",
                  color: mood === m ? "#fff" : "#292524",
                  borderColor: mood === m ? "#ec4899" : "#d6d3d1",
                }}
              >
                {m}
              </button>
          ...
          {showFavorites ? (
            favoriteRecipes.length === 0 ? (
              ...
              <div style={{ display: "grid", gap: 12 }}>
                {favoriteRecipes.map((recipe) => (
                  <div
                    key={recipe.name}
                    style={{ border: "1px solid #e7e5e4", borderRadius: 16, padding: 16 }}
                  >
            ...
            <p style={{ color: "#78716c", margin: 0 }}>
              Malzeme seçince veya filtre kullanınca öneriler burada görünür.
            </p>
          ) : (
            <div style={{ display: "grid", gap: 12 }}>
              {scoredRecipes.map((recipe) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      Puan: {Math.round(recipe.score)} | {recipe.category}
                    </span>
                  </div>
                  <p style={{ margin: "8px 0 0", color: "#57534e" }}>{recipe.text}</p>
                  {recipe.missing.length > 0 && (
                    <p style={{ margin: "8px 0 0", color: "#78716c", fontSize: 13 }}>
                      Markete lazım olanlar: {recipe.missing.join(", ")}
                    </p>
                  )}
                </div>
              ))}
